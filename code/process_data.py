...
def extract_text(original_dataset):
    ...
    for row in range(len(original_dataset.index)):
        ...
        try:
            ...
            article = Article(url=url, MIN_WORD_COUNT=300, fetch_images=False, http_success_only=True, language='en')
            article.download()
            article.parse()
...
def process_text (dataset):
    ...
    vectorizer = CountVectorizer(stop_words=stops, tokenizer=tokenizer, vocabulary=english_words)
